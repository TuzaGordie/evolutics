...
export class FindQuotationService {
  ...
  getProductClass(insuranceCodeCategory) {
    return this.http.get(this.baseURL + `codes/sub/category/product_class/${insuranceCodeCategory}`);
  }
  ...
  validate2(
    ...
    return this.http.get(
      this.baseURL + `policy/validate?clientNo=${clientNo}`
    );
  }

  findQuotation(searchParams: any) {
    let params = Object.entries(searchParams).reduce((query, [key, value]) => `${query}&${key}=${value}` , '?')
    return this.http.get( this.baseURL +
      `policy/quotes/search${params}`
    );
  }

  findQuotationByQuoteNo(
    quoteNo?: string
  ) {
    return this.http.get( this.baseURL +
      `policy/quotes/search?quoteNo=${quoteNo}`
    );
  }

  getDocument(data: any) {
    return this.http.get(
      this.baseURL+'client/document/' + data
    );
  }
  ...
  getProductsList() {
    return this.http.get(
      this.baseURL+'product/code/desc'
    );
  }
  ...
  getCreatorsList() {
    return this.http.get(
      this.baseURL+'users/id/fullname'
    );
  }

  getPolicyCodeByQuoteNo(quoteNo: string) {
    return this.http.get(
      this.baseURL+'policy/code/quote/no/' +
        quoteNo
    );
  }
  ...
  findQuote(parameters: Object) {
    ...
    return this.http.get(
      this.baseURL+'policy/quotes/search?' +
        query
    );
  }
