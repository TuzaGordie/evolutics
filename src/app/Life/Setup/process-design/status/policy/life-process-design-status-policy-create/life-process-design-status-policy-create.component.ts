...
@Component({
  selector: 'app-life-process-design-status-policy-create',
  templateUrl: './life-process-design-status-policy-create.component.html',
  styleUrls: ['./life-process-design-status-policy-create.component.scss'],
})
export class LifeProcessDesignStatusPolicyCreateComponent
  ...
  implements OnInit, AfterViewInit
{
  ...
  ngAfterViewInit(): void {
    super.init(async () => {
      if (this.code) {
        await this.fetchStatusCode(this.code);
        this.loading = false;
  ...
  ngOnInit(): void {
    ...
    this.codeService.getStatusCodeByCat('P').subscribe((data: any) => {
    ...
    //         this.fetchStatusCode(code, action)
    //         this.loading = false
    //       }
    //     }
    //   )
  ...
  addStatusDestination(i: number= this.createStatusCode.statusCodeDestination.length) {
    var dest = new StatusCodeDestination()
    dest.rowId = this.createStatusCode.statusCodeDestination.length + 1
    this.createStatusCode.statusCodeDestination.splice(i,0,dest)
  }
      ...
      if (this.util.confirm('Do you want to delete this status code?')) {
        this.codeService
          ...
          .subscribe(
            ...
            () => {
              this.util.notify('Unable to delete Status Destination.', 0);
            }
          );
      }
  ...
  saveStatusCode() {
    ...
    this.codeService.CreateStatusCode(this.createStatusCode, 'P').subscribe(
